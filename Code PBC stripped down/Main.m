...
file_path =  "/Users/paulabejaranocarbo/Documents/MATLAB/BCM_4est/" ; 
new_file_path = fullfile(file_path, "UK_data_hp.mat");
...
disp(T);
%%
writetable(T, 'Estimation_results_hp.xlsx', 'Sheet', 'Gamma_0.05_0.05_alphaeta2');

%% Iskrev local identification 


%% 

compare_ident_iskrev('/Users/paulabejaranocarbo/Documents/MATLAB/BCM_4est/BCM_4shocks_final_est/identification/BCM_4shocks_final_est_Current_params_identif.mat', ...
    '/Users/paulabejaranocarbo/Documents/MATLAB/BCM_4est/BCM_4shocks_final_est/identification/BCM_4shocks_final_est_posterior_mode_identif.mat')


%% 
% Ercolani table for prior

load('/Users/paulabejaranocarbo/Documents/MATLAB/BCM_4est/BCM_4shocks_final_est/identification/BCM_4shocks_final_est_Current_params_identif.mat')

J = double(ide_moments_point.dMOMENTS);     % Jacobian of theoretical moments wrt parameters

% clean parameter names
raw = store_options_ident.name_tex(:);
names = regexprep(raw,'^\s*\$|\$\s*$|\\_|\\|\{|\}|\^','');  % remove LaTeX
...
% Normalise only those
Jnorm = zeros(size(J));
Jnorm(:,nz) = J(:,nz) ./ col_norm(nz);

% Then compute the correlation matrix
Rho = Jnorm' * Jnorm;
% compute the full correlation/cosine matrix
Rho = Jnorm' * Jnorm;       % each entry = ρ_i,j

% find index of thetaG
idxG = find(strcmp(names,' thetaG '));
rho_thetaG = Rho(idxG,:);

% print correlation of thetaG with all other parameters
fprintf('Correlation ρ(thetaG, θ_j):\n');
for j = 1:numel(names)
    if j ~= idxG
        fprintf('  %-10s  %.3f\n', names{j}, rho_thetaG(j));
    end
end

disp(idxG)

%% Ercolani table for posterior 
load("/Users/paulabejaranocarbo/Documents/MATLAB/BCM_4est/BCM_4shocks_final_est/identification/BCM_4shocks_final_est_posterior_mode_identif.mat")
J = double(ide_moments_point.dMOMENTS);     % Jacobian of theoretical moments wrt parameters

% clean parameter names

names = param_names;  % remove LaTeX
