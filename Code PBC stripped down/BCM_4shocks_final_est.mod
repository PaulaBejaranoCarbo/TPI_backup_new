...
% features: BK fiscal extensions, capacity utilisation, inv adjustment costs, time-to-build delays 
% no time-varying labour income tax 

% shocks: productivity, marginal efficiency of investment, gc, ig 
...
steady; 
check; 
stoch_simul(order=1, nomoments, irf=100, periods = 5000, nograph); 

identification(order=1, parameter_set=prior_mean);
...
%%% Parameter Priors %%%
%%%%%%%%%%%%%%%%%%%%%%%%
eta, normal_pdf, 0.5, 0.1; //   not quite SW 
thetaG,  gamma_pdf,thetaG_prior_mean,thetaG_prior_sd;
kappa, normal_pdf, 4, 1.5; // SW
alpha,  beta_pdf, 0.33, 0.01; // SW do normal 
end; 
...
estimation(datafile=UK_data_hp, mode_compute=6, mh_replic=25000, mh_nblocks=2, mh_jscale=10, mh_drop=0.2, mode_check,nograph) Y Ip Gc Ig;

identification(order=1, parameter_set=posterior_mode);