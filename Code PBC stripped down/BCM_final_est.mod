...
set_param_value('rhoZ'      ,par.rhoZ);    // autocorrelation of X shock

set_param_value('h'        ,par.h);      // labor disutility
...
set_param_value('Profitsss', ss.Profits);

set_param_value('Lambdass', ss.lambda);

set_param_value('MUCss', ss.MUC);

rhoN = 0.9;

model;
#omegaAux = 1-(kappa/2)*(Ip/Ip(-1) -1)^2;
#markup = ((epsilon-1)/epsilon); 

% 1. Consumption FOC
lambda = (gamma/(1+tauC))*(MUC);

% 2. Euler equation for capital 
Q= bbeta*Q(+1)*(zeta(+1)/zeta)*(lambda(+1)/lambda)*((1-tauK)*Uk(+1)*Rk(+1) + Q(+1)*(1-deltaEff(+1))); 

% 3. LOM capital
Kp = (1-deltaEff)*Kp(-1) + Ip*omegaAux*X; 
...
% 16. FOC wrt Ip
1= Q*X*(omegaAux - kappa*(Ip/Ip(-1))*((Ip/Ip(-1)) -1)) + bbeta*Q(+1)*(zeta(+1)/zeta)*(lambda(+1)/lambda)*X(+1)*kappa*(Ip(+1)/Ip)^2*(Ip(+1)/Ip -1);
...
%20. Definition of MUC 
MUC = 1/(C-h*C(-1)) - (zeta(+1)/zeta)*bbeta*(h/(C(+1)-h*C)); 

%21. Process for Zeta 
log(zeta) = rhoZ*log(zeta(-1)) + epsZ; 

%22. Labour supply FOC 
lambda = (lshock*nu*N^eta)/(W*(1-tauN)); 

%23. labour shock process
log(lshock) = rhoN*log(lshock(-1)) + epsN; 
...
stderr epsZ, inv_gamma_pdf, 0.01, 0.2;
stderr epsN, inv_gamma_pdf, 0.01, 0.2;


%%% Parameter Priors %%%
%%%%%%%%%%%%%%%%%%%%%%%%
eta, normal_pdf, 0.3, 0.030; // BC 
thetaG,  beta_pdf, 0.05, 0.01;
kappa, normal_pdf, 4, 1.5; // SW
alpha,  beta_pdf, 0.3, 0.05; // SW 
h, beta_pdf, 0.7, 0.1; // SW
end; 

%_______________________%
% Observation Variables %
%‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾%
varobs hp_y, hp_i, hp_gcy, hp_igy, hp_h, hp_c;
estimation(datafile=UK_data_hp, mode_compute=6, mh_replic=25000, mh_nblocks=2, mh_jscale=10, mh_drop=0.2, mode_check) Y Ip Gc Ig;
